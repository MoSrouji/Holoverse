...
class PreferenceManager @Inject constructor(@ApplicationContext context: Context) {
    ...
    fun saveUser(user: User, isProfileComplete: Boolean = true) {
        val userJson = when (user) {
            ...
            is User.Admin -> json.encodeToString(user)
        }
    ...
    fun getUser(): User? {
        ...
        return try {
            when (userType) {
                UserType.Student.name -> json.decodeFromString<User.Student>(userJson)
                UserType.Mentor.name -> json.decodeFromString<User.Mentor>(userJson)
                UserType.Admin.name -> json.decodeFromString<User.Admin>(userJson)
                else -> null
            }
        } catch (e: Exception) {
            e.printStackTrace()
